...
function addLeadingZero(amount: string, shouldAllowNegative = false): string {
    ...
    return moneyRequestType === CONST.IOU.MONEY_REQUEST_TYPE.SPLIT || moneyRequestType === CONST.IOU.MONEY_REQUEST_TYPE.REQUEST;
}

/**
 * Validates that the sum of split amounts equals the total amount
 * @param splitDraftTransaction - The split draft transaction to validate
 * @returns Whether the split is valid
 */
function validateSplitAmount(splitDraftTransaction: Partial<Transaction>): boolean {
    if (!splitDraftTransaction || isEmptyObject(splitDraftTransaction)) {
        return true;
    }

    const {amount, splits} = splitDraftTransaction;
    ...
    return totalSplitAmount === amount;
}

export {getDefaultTaxCode, getTaxAmount, isValidMoneyRequestType, isAmountInvalid, isDistanceRequest, validateSplitAmount};
