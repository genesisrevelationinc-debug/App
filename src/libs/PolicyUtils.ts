...
import type {Policy, PolicyEmployee, Report, ReportAction, Transaction} from '@src/types/onyx';
import type {JoinWorkspaceResolution} from '@src/types/onyx/Policy';
import {isEmptyObject} from '@src/types/utils';
import {getDisplayNameOrDefault} from './UserUtils';
import type {BrickRoad} from './WorkspacesSettingsUtils';
import {getBrickRoadForPolicy} from './WorkspacesSettingsUtils';
...
/**
 * Get the display name of the policy owner to show in workspace list
 */
function getPolicyOwnerDisplayName(policy: OnyxEntry<Policy>): string | undefined {
    if (!policy) {
        return undefined;
    }

    // For pending join request workspaces, show the owner details
    if (policy.pendingJoinRequest) {
        const ownerEmail = policy.owner;
        if (ownerEmail) {
            return getDisplayNameOrDefault({displayName: ownerEmail, login: ownerEmail});
        }
    }
...
/**
 * Returns the policy name of the report
 */
            !!policy.id &&
...
function getActivePoliciesWithExpenseChatAndPerDiemEnabledAndHasRates(policies: OnyxCollection<Policy> | null, currentUserLogin: string | undefined): Policy[] {
    ...
    getActivePolicies,
    getActivePoliciesList,
    getPolicyNameByID,
    getPolicyOwnerDisplayName,
};
