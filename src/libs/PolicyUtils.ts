...
import type {Policy, PolicyEmployee, PolicyTag, PolicyTagList, PolicyTags, RecentlyUsedTags, Report, Transaction} from '@src/types/onyx';
import CONST from '@src/CONST';
...
function getCustomUnitsForDuplication(
    ...
    return policy?.areCompanyCardsEnabled ?? false;
}

/**
 * Check if a policy has any company card feeds with actual connection errors.
 * This filters out false positives from pending or transient states.
 */
function hasPolicyWithCompanyCardConnectionError(policy: OnyxEntry<Policy>, allFeeds: OnyxCollection<CompanyCardFeed> | undefined): boolean {
    if (!policy || !allFeeds) {
        return false;
    }
    
    const policyFeeds = policy?.companyCardFeeds ?? {};
    
    for (const [feedKey, feed] of Object.entries(policyFeeds)) {
        const fullFeed = allFeeds?.[`${ONYXKEYS.COLLECTION.COMPANY_CARD_FEED}${feedKey}`] ?? feed;
        if (CardUtils.hasCompanyCardFeedConnectionError(fullFeed)) {
            return true;
        }
    }
...
export {
    ...
    hasPolicyWithCompanyCardConnectionError,
};
 * Retrieves custom unit rate object from the given customUnitRateID
 */
