...
/**
 * Gets the default reimbursement account from a list of bank accounts.
 * In NewDot, the default account is determined by the `default` field.
 * This ensures consistency with how OldDot determines the default account.
 */
function getDefaultReimbursementAccount(bankAccounts: Record<string, BankAccount> | undefined): BankAccount | undefined {
    if (!bankAccounts || isEmptyObject(bankAccounts)) {
        return undefined;
    }

    const accounts = Object.values(bankAccounts);
    
    // First, look for an account explicitly marked as default
    const defaultAccount = accounts.find((account) => account?.default);
    if (defaultAccount) {
        return defaultAccount;
    }

    // Fallback: return the first account if no default is marked
    return accounts.at(0);
}

/**
 * Gets the default reimbursement account ID.
 */
function getDefaultReimbursementAccountID(bankAccounts: Record<string, BankAccount> | undefined): number | undefined {
    const defaultAccount = getDefaultReimbursementAccount(bankAccounts);
    return defaultAccount?.bankAccountID;
}

/**
 * Checks if a specific bank account is the default reimbursement account.
 */
function isDefaultReimbursementAccount(bankAccounts: Record<string, BankAccount> | undefined, bankAccountID: number): boolean {
    const defaultAccountID = getDefaultReimbursementAccountID(bankAccounts);
    return defaultAccountID === bankAccountID;
}

export {
    getDefaultReimbursementAccount,
    getDefaultReimbursementAccountID,
    isDefaultReimbursementAccount,
};
};
