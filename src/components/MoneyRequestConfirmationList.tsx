...
type MoneyRequestConfirmationListProps = {
    ...
    /** Whether the expense is a per diem expense */
        return selectedParticipants.every((participant) => participant.selected);
    }, [selectedParticipants]);

    const canUseWallet = useMemo(() => {
        return iouType === CONST.IOU.TYPE.SEND && 
               walletBalance !== undefined && 
               walletBalance > 0;
    }, [iouType, walletBalance]);

    const splitOrRequestOptions: ButtonWithDropdownMenuProps['options'] = useMemo(() => {
        const text = getSplitOrRequestOptionsText();
        return [
            ...
            ...(canUseWallet ? [
               {
                   ...
                   value: translate('iou.payWithWallet'),
            ...
            ...(iouType === CONST.IOU.TYPE.SEND ? [
                {
                    ...
                    value: translate('iou.markAsPaid'),
                    icon: Expensicons.Checkmark,
                },
            ] : []),
        ];
    }, [getSplitOrRequestOptionsText, canUseWallet, iouType, translate]);

    const selectedParticipantsAccountIDs = useMemo(() => selectedParticipants.map((participant) => participant.accountID), [selectedParticipants]);
